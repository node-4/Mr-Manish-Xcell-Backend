...
const createAgreement = async (req, res) => {
    try {
        ...
        if (!terms) {
            return res.status(400).json({ status: 0, message: "User or terms not found" });
        }
        const agreement = new Agreement({ userId, termsId, agreed_at: new Date(), });
        const updatedAgreement = await agreement.save();
        return res.status(201).json({ status: 1, message: "agreed terms and conditions", data: updatedAgreement, });
    } catch (err) {
        console.error(err);
...
const getAgreementsByUser = async (req, res) => {
    try {
        ...
        if (agreements.length === 0) {
            return res.status(200).json({ status: 0, message: "agreed terms and conditions not found", });
        }
